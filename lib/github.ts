...
**⚠️ Warning:** This PR was generated by Automata and requires human approval.
  ...
  const response = await fetch(
    `https://api.github.com/repos/${owner}/${repo}/pulls`,
    {
      ...
      body: JSON.stringify({
        ...
        head: branch,
        base: defaultBranch,
        draft: true, // Create as draft to wait for CI
      }),
    }
  );
  ...
  try {
    await fetch(
      `https://api.github.com/repos/${owner}/${repo}/issues/${pr.number}/labels`,
      {
        ...
        headers: getGitHubHeaders(),
        body: JSON.stringify({
          labels: ['automata-generated'],
        }),
      }
    );
  } catch (error) {
    // Log but don't fail if label addition fails
...
export async function createPRFromAICode(
  ...
  try {
    ...
    // Safety guards are applied in the webhook handler before calling this function
    ...
    try {
      branchSha = await createBranch(branchName);
      console.log(`Branch created with SHA: ${branchSha}`);
      // Audit log: Branch created
      logGitHubBranchCreated(ticket.key, branchName, branchSha);
    } catch (error) {
      const errorMessage = error instanceof Error ? error.message : String(error);
      logOperationFailed(ticket.key, AuditEventType.GITHUB_BRANCH_CREATED, errorMessage);
      throw error;
    }
    ...
    try {
      commitSha = await commitChanges(branchName, fileChanges, commitMessage);
      console.log('Changes committed successfully');
      // Audit log: Commit created
      logCommitCreated(ticket.key, commitSha, fileChanges.length);
    } catch (error) {
      const errorMessage = error instanceof Error ? error.message : String(error);
      logOperationFailed(ticket.key, AuditEventType.COMMIT_CREATED, errorMessage);
      throw error;
    }
    ...
    try {
      ...
      console.error('❌ Error creating PR:', prErrorMessage);
      
      // Check if PR already exists for this branch
      try {
        const { owner, repo } = getRepoInfo();
        const existingPRs = await fetch(
          `https://api.github.com/repos/${owner}/${repo}/pulls?head=${owner}:${branchName}&state=all`,
          { headers: getGitHubHeaders() }
        );
        ...
        if (existingPRs.ok) {
          ...
          if (prs.length > 0) {
            console.log(`⚠️  PR already exists for branch ${branchName}: #${prs[0].number}`);
            return {
        ...
        console.warn('Could not check for existing PR:', checkError);
      }
      
      // If PR creation failed and no existing PR found, throw error
      throw new Error(`Failed to create PR: ${prErrorMessage}`);
    }
  } catch (error) {
    const errorMessage = error instanceof Error ? error.message : String(error);
