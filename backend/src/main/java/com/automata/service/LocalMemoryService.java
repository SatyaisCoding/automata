...
public class LocalMemoryService {
    ...
    public void updateIncidentCIStatus(String id, String ciStatus) {
        try {
            File file = new File(memoryFilePath);
            if (!file.exists()) return;

            String content = new String(Files.readAllBytes(Paths.get(memoryFilePath)));
            ArrayNode rootArray = (ArrayNode) objectMapper.readTree(content);

            boolean updated = false;
            for (int i = 0; i < rootArray.size(); i++) {
                ObjectNode incident = (ObjectNode) rootArray.get(i);
                if (incident.path("id").asText().equals(id)) {
                    JsonNode fixes = incident.path("fixes");
                    if (fixes.isArray() && fixes.size() > 0) {
                        ObjectNode firstFix = (ObjectNode) fixes.get(0);
                        firstFix.put("ci", "success".equals(ciStatus) ? "Passed" : ("failure".equals(ciStatus) ? "Failed" : "Pending"));
                        updated = true;
                    }
                }
            }
            ...
            if (updated) {
                objectMapper.writerWithDefaultPrettyPrinter().writeValue(file, rootArray);
                log.info("[SUCCESS] Updated incident {} CI status to {} in persistent local memory database.", id, ciStatus);
            }
        } catch (Exception e) {
            log.error("[ERROR] Failed to update incident CI status in local memory database: {}", e.getMessage(), e);
        }
    }
